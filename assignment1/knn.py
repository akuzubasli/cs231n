...
# see http://stackoverflow.com/questions/1907993/autoreload-of-modules-in-ipython test
...
X_train = np.reshape(X_train, (X_train.shape[0], -1)) #-1 means the unspecified value is to be inferred as 32*32*3
...
for k in k_choices: # for each k
    for n in range(num_folds): # for each nth fold
        ...
        X_train_other_folds = np.concatenate((X_train_folds[other_folds]), axis=0)
        y_train_other_folds = np.concatenate((y_train_folds[other_folds]), axis=0)
        classifier = KNearestNeighbor()
