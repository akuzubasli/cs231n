...
#This function is very similar to the loss functions 
#you have written for the SVM and Softmax exercises: 
#It takes the data and weights and computes the class scores, 
...
# to plot the loss function and the accuracies on the training and validation sets during optimization.
# 
...
# 
# **What's wrong?**. Looking at the visualizations above, 
...
# is a large part of using Neural Networks, so we want you to get a lot of practice. 
...
for hid_size in hidden_size:
    ...
    for lr in learning_rates:
        for reg in regularization_strengths:
            for num_iter in number_of_epochs:
                tic = time.time()
                net = TwoLayerNet(input_size, hidden_size, num_classes)
                training_results = net.train(X_train, y_train, X_val, y_val,
                ...
                est_time = toc - tic
                results[(hid_size, lr, reg, num_iter)] = (val_acc, est_time)
                if val_acc > best_val:
                    best_val = val_acc
                    best_net = net
...
# Print out results.
for hid_size, lr, reg, num_iter in sorted(results):
    val_accuracy, time = results[(hid_size, lr, reg, num_iter)]
    print('hid_size %d lr %.2E reg %.2e num_iter %d - val accuracy: %f - time: %fs' % (
                hid_size, lr, reg, num_iter, val_accuracy, est_time))
    
print('BEST val accuracy: %f with hid_size %d lr %.2E reg %.2e num_iter %d' % 
          (best_val, best_hid_size, best_lr, best_reg, best_num_iter))
