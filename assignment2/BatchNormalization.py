...
#  However even if we preprocess the input data, the activations at deeper layers of the network will likely no longer be decorrelated 
#  and will no longer have zero mean or unit variance since they are output from earlier layers in the network. 
#  Even worse, during the training process the distribution of features at each layer of the network will shift 
#  as the weights of each layer are updated.
# 
#  The authors of [3] hypothesize that the shifting distribution of features inside deep neural networks 
#  may make training deep networks more difficult. To overcome this problem, 
...
# In the file `cs231n/layers.py`, implement the batch normalization forward pass in the function `batchnorm_forward`. 
...
# In class we talked about two different implementations for the sigmoid backward pass. 
#  One strategy is to write out a computation graph composed of simple operations and backprop through all intermediate values. 
#  Another strategy is to work out the derivatives on paper. For the sigmoid function, 
#  it turns out that you can derive a very simple formula for the backward pass by simplifying gradients on paper.
# 
# Surprisingly, it turns out that you can also derive a simple expression for the batch normalization backward pass 
#  if you work out derivatives on paper and simplify. 
#  After doing so, implement the simplified batch normalization backward pass in the function 
#  `batchnorm_backward_alt` and compare the two implementations by running the following. 
...
#  The second layer will plot training accuracy, validation set accuracy, and training loss 
#  as a function of the weight initialization scale.
...
# # Answer: The accuracy and loss are less sensitive to weight initialization in batchnorm case as 
# it first makes the inputs to the relu layers, first zero-mean and unit-variance then scales 
# and shifts it with learnable parameters (gamma and beta). Moreover, batchnorm introduces stochasticity as it operates on batches. 
# Thus, although it may have lower training value, it helps to regularization and results in better validation accuracy.
# 
